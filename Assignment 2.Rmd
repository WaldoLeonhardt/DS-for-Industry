...
To illustrate this, a simple tree model is fitted on the SONA data, using the president's name as the "document" identifier. This yielded a training accuracy of 42% and at test accuracy of 41%. (Better models can be built using the bag-of-words dataset, as is shown later in this document.) 
...
plot(bow_fit, main="Full Classification Tree using Bag-of-Words")
...
# predtrain
kable(predtrain,
      caption = "Training confusion matrix for classification Tree using Bag-of-Words.",
      row.names = TRUE, digits = 0)
cat("Training classification success rate = ", sum(diag(predtrain))/sum(predtrain), "\n") # training accuracy
```
...
kable(predtest,
      caption = "Test confusion matrix for classification Tree using Bag-of-Words.",
      row.names = TRUE, digits = 0)
cat("Test classification success rate = ", sum(diag(predtest))/sum(predtest),"\n") # test accuracy
```
...
Showing the **tf**, **idf** and **tf_idf** information added to the Sona words tibble below.
...
options(repr.plot.width=8, repr.plot.height=10)
plot(fit_tfidf, main="Full Classification Tree with BoW TF-IDF")
...
kable(predtrain_tfidf,
      caption = "Training confusion matrix for classification Tree using Bag-of-Words with TF-IDF.",
      row.names = TRUE, digits = 0)
cat("Training classification success rate = ", sum(diag(predtrain_tfidf))/sum(predtrain_tfidf),"\n") # training accuracy
```
...
kable(predtest_tfidf,
      caption = "Test confusion matrix for classification Tree using Bag-of-Words with TF-IDF.",
      row.names = TRUE, digits = 0)

cat("Test classification success rate = ", sum(diag(predtest_tfidf))/sum(predtest_tfidf),"\n") # test accuracy
```
...
We can see our test accuracy reduced slightly from 49.2% to 49%, and the model is still overfitting.

We can also look at the confusion matrix:

```{r}
pres_predicted2 = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(model2$predict_classes(x_test), (0:5))]
pres_actual = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(predicter_presidents_id[-train.index], (0:5))]

kable(table(predicted = pres_predicted2, actual = pres_actual),
      caption = "Training confusion matrix for Feed-forward neural network",
      row.names = TRUE, digits = 0)
...
* deKlerk - only 1 predictions were correct. This can be expected as we had the least number of words for deKlerk (only 1% of all words were assosiated with him).
* Ramaphosa - only 4 predictions were correct. This is also expected as only 2.9% of all words were assosiated with him.
* Zuma - where we had most predictions correct and we also had 30.9% of all words assosiated with him.
* Mbeki - for whom we had the highest word percentage assosiation (39.1%) and where more than half of the predictions were correct.
...
To asses if any improvement can be gained through the use of TF-IDF over standard word frequency data, a feed-forward neural network, is used to predict which president used a particular sentence in his SONA. The data is split up into a test and train set (70% to 30%).  
...
sona_bow_tfidf_nr = sona_bow_tfidf %>%
  left_join(pres_lookup, by = "president_sona") %>%
  select(president_sona, pres_nr, sentence.id, everything())

# unique(sona_bow_tfidf_nr$pres_nr)

train_ids <- sona_bow_tfidf_nr %>% 
  group_by(president_sona) %>% 
  sample_frac(0.7) %>% 
  ungroup() %>%
  select(sentence.id)

train_bow_tfidf <- sona_bow_tfidf_nr %>%
  ungroup() %>%
  right_join(train_ids, by = "sentence.id") %>%
  select(-sentence.id)
...
test_bow_tfidf <- sona_bow_tfidf_nr %>%
  ungroup() %>%
  anti_join(train_ids, by = "sentence.id") %>%
  select(-sentence.id)
...
train_bow_tfidf_x = select(train_bow_tfidf,-pres_nr, -president_sona, -year_sona, -count_sona)
train_bow_tfidf_x = as.matrix(train_bow_tfidf_x)
# train_bow_tfidf_x[1:10,1:10]
...
```{r}
#model7 = load_model_hdf5("models/FE_SONA_model7.h5") #save model to use later on
#load_model_weights_hdf5(model7, "models/FE_SONA_model7_weights.h5") #save model weights to use later on
#load("models/history7.RData")
#load("models/NN_TFIDF.RData")
```
...
As can be seen from the reduced train and test accuracy above, TF-IDF did not enhance the predictive capability in this Feed-forward neural network.
...
* deKlerk - we now have 6 correct prediction compared with 1 before.
* Ramaphosa - we now also have 6 correct prediction compared with 4 before.
...
```{r}
pres_predicted3 = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(model3$predict_classes(x_test), (0:5))]

kable(table(predicted = pres_predicted3, actual = pres_actual),
      caption = "Training confusion matrix for Feed-forward neural network (correcting class imbalance)",
      row.names = TRUE, digits = 0)
...
hist(sentence_word_counts$count, breaks = 40, col = "lightgrey", main = "Histogram of sentence word counts", xlab = "Total words in sentence", xlim = c(0,130))
...
hist(word_counts$count, breaks = 80, col = "lightgrey", main = "Histogram of word count occurences", xlab = "Word count", xlim=c(0,300))
```
...
Model 4 is a good improvement on the previous models. We now have an accuracy of 59.2% where the previous best was 49.2%
...
cnn_parameter_tests$Test.Accuracy = round(cnn_parameter_tests$Test.Accuracy,4) * 100
...
```{r}
pres_predicted4.1 = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(model4$predict_classes(x_test_cnn), (0:5))]
pres_actual_cnn = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(test_cnn$y, (0:5))]

kable(table(predicted = pres_predicted4.1, actual = pres_actual_cnn),
      caption = "Training confusion matrix for CNN (Approach 1)",
      row.names = TRUE, digits = 0)

```

```{r}
# Prediction accuracy per president
pres_predicted4.1 = data.frame(round(diag(table(predicted = pres_predicted4.1, actual = pres_actual_cnn)) / table(pres_actual_cnn), 4) * 100)
colnames(pres_predicted4.1) = c("President", "Accuracy")
pres_predicted4.1
```
...
```{r}
pres_predicted4.2 = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(model4$predict_classes(x_test_cnn), (0:5))]

kable(table(predicted = pres_predicted4.2, actual = pres_actual_cnn),
      caption = "Training confusion matrix for CNN (Approach 2)",
      row.names = TRUE, digits = 0)
```

```{r}
# Prediction accuracy per president
pres_predicted4.2 = data.frame(round(diag(table(predicted = pres_predicted4.2, actual = pres_actual_cnn)) / table(pres_actual_cnn), 4) * 100)
colnames(pres_predicted4.2) = c("President", "Accuracy")
pres_predicted4.2
```
...
```{r}
#model4.3 = load_model_hdf5("models/Waldo_SONA_model4.3.h5")
#load_model_weights_hdf5(model4.3, "models/Waldo_SONA_model4.3_weights.h5")
#load("models/history4.1.RData")
#load("models/history4.2.RData")
#load("models/history4.3.RData")
```
...
```{r}
pres_actual4.3 = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(test_cnn$y, (0:5))]

pres_predicted4.3 = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(model4.3$predict_classes(x_test_cnn), (0:5))]

kable(table(predicted = pres_predicted4.3, actual = pres_actual4.3),
      caption = "Training confusion matrix for CNN (Approach 3)",
      row.names = TRUE, digits = 0)
...
```{r}
# Prediction accuracy per president
pres_predicted4.3 = data.frame(round(diag(table(predicted = pres_predicted4.3, actual = pres_actual4.3)) / table(pres_actual4.3), 4) * 100)
colnames(pres_predicted4.3) = c("President", "Accuracy")
pres_predicted4.3
```
Note on above: The accuracy for Ramaphosa and Zuma should be added and allocated to Zuma as there was no predictions for Ramaphosa. This allocation occured incorrectly when this table was build.
...
Approach 3 has a good average accuracy of 58.2% but no predictions were made for Ramaphosa, therefore we will not select this option.
...
This model only achieved an accuracy of 54.6% which is not better than our first CNN model which achieved 59.2%. It seems that a more general approach worked better than to add to many extra layers.


## RNN

We can also look at a Recurrent Neural Network (RNN) which is similar to a CNN, where the output depends not only on the present inputs but also on the previous step’s neuron state. The order of words is preserved by using long short-term memory. We can also use Keras to build a RNN and use the Long Short-Term Memory (LSTM) layer.
...
The RNN is fairly simple model and have our best accuracy when compared to our previous models (59.7% compared to 59.2%).
...
From the above random tests, we have 4 correct and 1 incorrect predictions which is better than our average 59.7% accuracy for the CNN model.


We can observe the confusion matrix for this RNN model from which we can see that the model gets confused in the following ways:

* Predict Mandela correctly most of the time (43.6%), but confuse him with Mbeki 38% of the time.
* Mostly thinks Motlanthe is Mbeki (42.3% of the time), while only predicting Motlanthe correctly 3.8% of the time.
* Fail to predict Ramaphoza and thinks he is Zuma 46.7% of the time.

```{r}
pres_actual_rnn = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(test_cnn$y, (0:5))]

pres_predicted_rnn = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa")[ match(model6$predict_classes(x_test_cnn), (0:5))]

kable(table(predicted = pres_predicted_rnn, actual = pres_actual_rnn),
      caption = "Training confusion matrix for RNN",
      row.names = TRUE, digits = 0)

```

We can also compare the baseline accuracy with the validation prediction accuracy.

```{r}
model6_summary = data.frame(matrix(NA, nrow = 6, ncol = 0))
model6_summary$Total.Sentences = table(sona_sentences$president)
model6_summary$Baseline.Accuracy = table(sona_sentences$president) / nrow(sona_sentences)
model6_summary$Prediction.Accuracy = diag(table(predicted = pres_predicted_rnn, actual = pres_actual_rnn)) / table(pres_actual_rnn)
rownames(model6_summary) = rownames(table(sona_sentences$president))

model6_summary$Baseline.Accuracy = round(model6_summary$Baseline.Accuracy, 4) * 100
model6_summary$Prediction.Accuracy = round(model6_summary$Prediction.Accuracy, 4) * 100

kable(model6_summary,
      caption = "RNN model summary",
      row.names = TRUE, digits = 0)
...
To predict which president said a sentence, the training data was split into a train and test set. Taking a random guess for our dataset of six presidents, given a sentence, one has a chance of 16.67% (1 out of 6) to correctly guess the president who used the sentence in his SONA. A smarter guess would be to look at the distribution of sentences, then guessing always Zuma, would give an accuracy of 36.3%. So the aim with our neural networks was to beat these chances. The following neural network models were built to predict the president from any given sentence extracted from a SONA speech:

* Feed-forward neural network (general) with an accuracy of 49.2%
* Feed-forward neural network (with tf-idf) with an accuracy of 22%
* Convolutional Neural Network (with embeddings) with an accuracy of 59.2%
* Recurrent Neural Network with an accuracy of 59.7%

The feed-forward neural networks did not perform very as well they do not preserve the order of words within sentences. Therefore, the convolutional and recurrent neural networks performed much better where the order of words are preserved. Correction of class imbalances were also attempted but this did not provide any significant improvement, and therefore a decision was made to keep class data as provided.

Our final choice for a model to predict a president is the recurrent neural network as it is fairly simple and had the best performance.
...
|Waldo Leonhardt    | Feed-forward neural net, CNN and RNN with hyperparameter tuning        |
|Francois Evert     | Bag of words, Bigrams with negation, TF-IDF for feed-forward neural net|
