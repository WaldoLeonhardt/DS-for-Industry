...
sona_words = sona_words %>% 
  rename(president_sona = president, year_sona = year, word_sona = word, count_sona = count)
 

# Using built in function (https://www.tidytextmining.com/tfidf.html)
sona_words_tfidf = sona_words %>%  bind_tf_idf(word_sona, sentence.id , count_sona)
# head(sona_words)
# dim(sona_words)

# sona_words %>% arrange(desc(tf_idf))
# head(arrange(sona_words,desc(tf_idf)))
...
sona_words_tfidf %>%
  ...
  top_n(15) %>% 
  ungroup %>%
  ggplot(aes(x = reorder(word_sona,tf_idf,max), y = tf_idf, fill = president_sona)) +
...
options(repr.plot.width=8, repr.plot.height=10)
plot(fit_tfidf, main="Full Classification Tree")
text(fit_tfidf, use.n=TRUE, all=TRUE, cex=.8)


fittedtrain_tfidf <- predict(fit,type="class")
...
fittedtest_tfidf <- predict(fit,newdata=test_bow_tfidf,type="class")
...
beta_spread %>%
  ...
  geom_col() +
  coord_flip()
>>>>>>> 909b2a5840d7e18d52156b2aa5e0ce2a5be74854
