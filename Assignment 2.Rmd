...
We can see our test accuracy improved slightly from 46% to 48.6%, but the model is still overfitting.
...
* deKlerk (ID = 0), only 1 predictions were correct. This can be expected as we had the least number of words for deKlerk (only 1% of all words were assosiated with him).
* Ramaphosa (ID = 5), not a single prediction was correct. This is also expected as only 2.9% of all words were assosiated with him.
...
Note our overall performance has not improved and sit on 47.8%.

Looking at the confusion matrix we can however see the following improvement:

* deKlerk (ID = 0), we now have 3 correct prediction compared with 0 before.
* Ramaphosa (ID = 5), we now have 4 correct prediction compared with 2 before.
...
Model 4 is a good improvement on the previous models. We now have an accuracy of 55.2% where the previous best was 47.8%
