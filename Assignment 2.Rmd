...
Confirm the number of presidents, and list them in the order of total words we have for them.
...
If we consider the most used words, we can roughly interpret our topics as below:  
 - Topic 1 has $Governance$, $Programme$, $Nation$, $Develop$ and $Sector$ as important words. The unique words here (compared to top 5 words in other topics are $sector$ and $programme$.) Hence our conclusion is that this topic concerns governing by developing sectors and programmes. 
 -Topic 2 has $South$, $people$, $African$, $Nation# and $Countries$ as the top 5 words. The combination of these words tell us that this topic has to do with the identity of the country. The only unique word here in the top 5 compared to other topics, is $African$.  
 -Topic 3 has words in the top 5 of $Country$, $Govern$, $Develop$, $Improve$ and $People$. Unique words we see here is $Improve$ hence we conclude that this topic refers to country improvement and development.  
 -Topic 4 has words of $Govern$, $People$, $Country$, $Ensure$ and $Include$ in its top 5 words. The unique words here are $include$ and $ensure$ which tells us that this topic is about inclusion in the country and ensuring that things are done.  
 -Topic 5 has the words $Govern$, $Nation$, $People$, $South$ and $Constitution$ coming out in the top 5. Unique words here refer to $Constitution$ hence we can conclude that this topic refers to the country's constitution.  
 -Topic 6 have top 5 words as $Govern$, $South$, $Nation$, $Develop$ and $Continuous$ and we notice the unique words here are $Continuous$ so we can see this topic as one that states that things must continue to develop for the nation. 
...
Topic 1 (Development and programmes) differs from topic 2 (African/country identity) the most by words like $even$ from topic 2 and $discharge$ seen mostly in topic 1. Topic 1 also include words like $truth$ and $instances$ which makes sense considering it is about development and programmes as opposed to identity. 
Topic 3 (Country improvement and development) differs from topic 1 with words such as $Diligent$ and $Even$ used in topic 3 and topic 1 has more words regarding $Centenary$ and $own$ used that we do not see in topic 3.
Topic 4 (Inclusion and ensuring) also inclused words like $diligent$ and $even$ more than topic 1. This is expected as topic 4 focuses on inclusion, hence $even$ and fairness will be important in this topic, much more than in a topic regarding development and programmes.
Topic 5 (Constitution) differs also by using words like $even$ and $diligent$ but it contains far fewer occurences of topic 1 words like $compatriot$, $limpopo$ and $graduation$.
Topic 6 (Continuous) contains words like $even$ and $diligence$, but fewer $discharge$ and $government$ words than topic 1. 
...
De Klerk's speech consists mainly of topic 5 (Constitution.) This makes sense considering this is the last speech before apartheid ended and many things changed constitutionally thereafter (as well as enabling all people to vote.) 

Mandela's speech used both topic 3 (less) and topic 5 (higher percentage) with his speeches. Topic 3 covers the country improvement and development and topic 5 the constitution. It is interesting to note that both de Klerk and Mandela had topic 5 covered, as this was a time for transition for the country and the constitution would have been an important topic to cover considering the changes made in 1994. 

Topic 1 (Development and programmes) makes up a portion of Mbeki's speech. We also see Topic 4 (Inclusion and ensuring) making up the larger part of Mbeki's speech. Mbeki's speech can hence be identified by these 2 main topics, we do not see other presidents making use of these topics significantly.

Motlanthe took over from Mbeki and we see his speech mainly consisted of topic 3 (country improvement and development.) It is interesting to note that both he and Mandela used topic 3 and both times these presidents would have viewed the previous presidency as something that had to be 'corrected.' (De Klerk, Mandela's predecessor, was from another party,  and Mbeki was ousted early as president as well.) We see a small percentage of Motlanthe's speech covering topic 2 (African country/identity) as well. This is interesting to note as his successor, Zuma, also had a portion of this topic regarding African country/identity in his speech. 

Zuma's speeches mainly covered topic 6 (continuous) but we see his successor, Ramaphose, only covered topic 2 (African country/identity.) So the past 3 presidents (Motlanthe, Zuma and Ramaphosa) all had an element of the African country/identity in their speeches, but Motlanthe focused on this far more than his predecessors. 
...
We can do the same for the negative words, which president used the mostly used negative words and how many times during his reigh as president?  
...
## Bag of Words
Bag-of-Words is structured representation of the words used in sets of documents (*corpus*). In the "Bag", the order, punctuation and subtle implication ("between the lines" meaning) of each word is disregarded, and merely the occurrence count per document is maintained in the representation. Such Bag-of-words information sets and often used as input to descriptive and predictive modelling. 

To illustrate this, a simple tree model is fitted on the SONA data, using the president's name as the "document" identifier. Which yielded a training accuracy of 43% and at test accuracy of 42%. (Better models can be build using the bag-of-words dataset, as is shown later in this document.)
...
fit <- rpart(factor(president_sona) ~ ., train_bow)
...
fittedtest <- predict(fit,newdata=test_bow,type="class")
...
Again, to illustrate the use, a simple tree model is fitted on the SONA data, using tf-idf in this case, and the president's name as the "document" identifier. In this case marginally better training accuracy of 44% and at test accuracy of only 10% were achieved.
(Note that the number of words were limited to the top 2000 based on TF-IDF.)
...
```{r}
cat("Train classification success rate = ", mean(model1$predict_classes(train_bow_tfidf_x) == train_bow_tfidf_y), "\n")
cat("Test classification success rate = ", mean(model1$predict_classes(test_bow_tfidf_x) == test_bow_tfidf_y))
```
...
The test accuracy is also worse than what a random guess would have been (e.g. 50%).
...
We will start with checking sentence lengths. In other words what is the average number of words per sentence and what is the maximum lenght of sentences we should consider to reprecent 98% of the data?
...
hist(sentence_word_counts$count, col = "lightgrey", main = "Histogram of sentence word counts", xlab = "Total words in sentence")
```

```{r}
cat("average number of words per sentence is", mean(sentence_word_counts$count), "\n")
cat("maximum lenght of sentences which reprecent 98% of data is", quantile(sentence_word_counts$count, 0.98), "\n\n")
...
Check frequency of words accross all sentences to help determine the maximum features to use. In other words, how many unique words should we keep to sill reprecent 95% of the data?
...
hist(word_counts$count, col = "lightgrey", main = "Histogram of word count occurences", xlab = "Word count")
```
...
We can try and make our CNN model better by adding more convolutional layers. For the next model we will use the same basis as for model4 by keeping the same max_features, minlen, maxlen and embedding_dims.
...
This model only achieved an accuracy of 54.5% which is not better than our first CNN model which achieved 56.9%. It seems that a more general approach worked better than to add to many extra layers.


## RNN

We can also look at a Recurrent Neural Network (RNN) which is similar to a CNN, where the output depends not only on the present inputs but also on the previous step’s neuron state. We can also use Keras to build a RNN and use the Long Short-Term Memory (LSTM) layer.
...
The RNN is fairly simple model and have a reasonable accuracy when compared to our previous models.
...
```{r}
# select 5 random test sentence
test_index = sample((1:length(sequences))[-train.index.main], size = 5, replace = F)
...
for (i in test_index)
{
  ...
  cat("Predicted president:", sona_sentences[i,]$president, "\n\n")
}
```
