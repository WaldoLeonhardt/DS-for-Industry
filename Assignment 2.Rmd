...
model7 %>% 
  layer_dense(units = 0.6*input_shape,                     # number of neurons in 1st hidden layer, use 2/3 of (input_shape + output_shape)
              activation = 'relu',              # use a rectified linear unit as an activation function in 1st hidden layer
              input_shape = input_shape) %>%    # dimension of input array
  layer_dropout(rate = 0.3) %>%                 # add dropout with rate 30%
...
Approach 3 has a good average accuracy of 59.3% but no predictions were made for Ramaphosa, therefore we will not select this option.
...
From the above random tests, we have 4 correct and 1 incorrect predictions which is better than our average 59.2% accuracy for the model.
...
* Mostly thinks de Klerk is Mandela (50% of the time), while only predicting de Klerk correctly 37.7% of the time.
* Predict Mandela correctly most of the time (44.4%), but confuse him with Mbeki 37.5% of the time.
* Fail to predict Ramaphoza and thinks he is Zuma 70.6% of the time.
...
* Feed-forward neural network (general) with an accuracy of 49%
* Feed-forward neural network (with tf-idf) with an accuracy of 33.9%
* Convolutional Neural Network (with embeddings) with an accuracy of 59.1%
* Recurrent Neural Network with an accuracy of 59.2%
